...
# Description : Generate a Vivado Block Design project with HelmHoltz kernel,
...
if { $kernel_num < 1 || $kernel_num > 14 } {
  puts "ERROR: Invalid kernel number $kernel_num! < 2 or > 15"
  return
}
...
# Create kernel_num krnl_helm_X
for {set i 0} {$i < $kernel_num} {incr i} {
  # Create instance: krnl_helm_X, and set properties
  set krnl_helm_tmp [ create_bd_cell -type ip -vlnv xilinx.com:hls:krnl_helm:1.0 krnl_helm_$i ]
  set_property -dict [ list \
...
for {set i 0} {$i < $kernel_num} {incr i} {
  connect_bd_intf_net -intf_net S00_AXI_${i} [get_bd_intf_pins axi_mem_intercon_${i}/S00_AXI] [get_bd_intf_pins krnl_helm_${i}/m_axi_gmem0]
  connect_bd_intf_net -intf_net S01_AXI_${i} [get_bd_intf_pins axi_mem_intercon_${i}/S01_AXI] [get_bd_intf_pins krnl_helm_${i}/m_axi_gmem1]
}
...
for {set i 0} {$i < $kernel_num} {incr i} {
  ...
  connect_bd_intf_net -intf_net smartconnect_0_M${ifn}_AXI [get_bd_intf_pins krnl_helm_${i}/s_axi_control] [get_bd_intf_pins smartconnect_0/M${ifn}_AXI]
}
...
for {set i 0} {$i < $kernel_num} {incr i} {
  set axin [format "%02u" [expr {$i + 1}]]
  connect_bd_net -net qdma_0_axi_aclk [get_bd_pins krnl_helm_${i}/ap_clk]
...
for {set i 0} {$i < $kernel_num} {incr i} {
  set axin [format "%02u" [expr {$i + 1}]]
  connect_bd_net -net qdma_0_axi_aresetn [get_bd_pins krnl_helm_${i}/ap_rst_n]
...
for {set i 0} {$i < $kernel_num} {incr i} {
  ...
  for {set j 0} {$j < 16} {incr j} {
    ...
    assign_bd_address -offset $hbm_addr -range 0x20000000 -target_address_space [get_bd_addr_spaces krnl_helm_${i}/Data_m_axi_gmem0] [get_bd_addr_segs hbm_0/SAXI_${axin}_8HI/HBM_MEM${mem_num}] -force
    assign_bd_address -offset $hbm_addr -range 0x20000000 -target_address_space [get_bd_addr_spaces krnl_helm_${i}/Data_m_axi_gmem1] [get_bd_addr_segs hbm_0/SAXI_${axin}_8HI/HBM_MEM${mem_num}] -force
  }
  ...
  if { $hbm_ctrl_num eq 2 } {
    for {set j 0} {$j < 16} {incr j} {
      ...
      set mem_num [format "%02u" $j]
      assign_bd_address -offset $hbm_addr -range 0x20000000 -target_address_space [get_bd_addr_spaces krnl_helm_${i}/Data_m_axi_gmem0] [get_bd_addr_segs hbm_1/SAXI_${axin}_RT_8HI/HBM_MEM${mem_num}] -force
      assign_bd_address -offset $hbm_addr -range 0x20000000 -target_address_space [get_bd_addr_spaces krnl_helm_${i}/Data_m_axi_gmem1] [get_bd_addr_segs hbm_1/SAXI_${axin}_RT_8HI/HBM_MEM${mem_num}] -force
    }
  }
}
...
for {set i 0} {$i < $kernel_num} {incr i} {
  ...
  assign_bd_address -offset ${kern_addr} -range 0x00000080 -target_address_space [get_bd_addr_spaces qdma_0/M_AXI] [get_bd_addr_segs krnl_helm_${i}/s_axi_control/Reg] -force
}
