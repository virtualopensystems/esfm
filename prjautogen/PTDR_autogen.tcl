...
# Description : Generate a Vivado Block Design project with PTDR kernels,
#               launch synthetesis and implementation, exporting the bitstream
#               file when finished.
################################################################################

# Change Design options here
set prj_name "PTDR14HBM2"
set kernel_num 14
set use_secondary_HBM 1
set num_jobs 16
...
for {set i 0} {$i < $kernel_num} {incr i} {
    ...
    set krnl_tmp [ create_bd_cell -type ip -vlnv user.org:user:krnl_ptdr:1.0 krnl_$i ]
    #set_property -dict [ list \
...
for {set i 0} {$i < $kernel_num} {incr i} {
    ...
    for {set j 0} {$j < 16} {incr j} {
        ...
        assign_bd_address -offset $hbm_addr -range 0x20000000 -target_address_space [get_bd_addr_spaces krnl_${i}/m_axi_gmem0] [get_bd_addr_segs hbm_0/SAXI_${axin}_8HI/HBM_MEM${mem_num}] -force
    }
    ...
    if {$hbm_ctrl_num eq 2} {
        for {set j 0} {$j < 16} {incr j} {
            ...
            set mem_num [format "%02u" $j]
            assign_bd_address -offset $hbm_addr -range 0x20000000 -target_address_space [get_bd_addr_spaces krnl_${i}/m_axi_gmem0] [get_bd_addr_segs hbm_1/SAXI_${axin}_RT_8HI/HBM_MEM${mem_num}] -force
        }
    }
}
...
for {set i 0} {$i < $kernel_num} {incr i} {
    ...
    assign_bd_address -offset ${kern_addr} -range 0x00000080 -target_address_space [get_bd_addr_spaces qdma_0/M_AXI] [get_bd_addr_segs krnl_${i}/s_axi_control/reg0] -force
}
